def enumerate_ask(target_variable, observations, bayes_net, variables):
    '''
    target_variable = Single variable name -> NightDefense
    observations = given events -> {NightDefense : +, Infiltration : - }
    bayes_net - Dictionary of EventNode 
    '''
     
    calc_prob = {} # required probability numbers for target_variable
#     hidden_variables = [v for v in bayes_net.keys() if v != target_variable and v not in observations.keys()]
    
    # TODO check if observations could be provided for nodes with parents
        
    # TODO make xi restricted to asked values
    for xi in ["+","-"]:
        print target_variable, xi, observations
        calc_prob[xi] = enumerate_all(variables, extend(observations, target_variable, xi), bayes_net)
